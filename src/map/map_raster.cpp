/* Raster tile map — ESP32 create_map_screen() + region discovery ported to Linux
 * Stations APRS : icônes positionnées sur les tuiles, clic → popup info
 */
...
static const char *mapsRoot() {
    ...
    const char *candidates[] = {
        ...
        "/media/fab2/TILES/LoRa_Tracker/Maps",
        "/data/LoRa_Tracker/Maps",
        NULL
    };
    for (int i = 0; candidates[i]; i++) {
        struct stat st;
        if (stat(candidates[i], &st) == 0) { strncpy(root, candidates[i], sizeof(root)-1); return root; }
    }
    return NULL;
}

static void discoverRegion() {
    if (mapRegion[0]) return;
    ...
    while ((e = readdir(d))) {
        if (e->d_type == DT_DIR && e->d_name[0] != '.') {
            strncpy(mapRegion, e->d_name, sizeof(mapRegion)-1); break;
        }
    }
    closedir(d);
}
...
static void discoverZooms() {
    if (!mapRegion[0]) { discoverRegion(); if (!mapRegion[0]) return; }
    char zpath[512]; snprintf(zpath, sizeof(zpath), "%s/%s", mapsRoot(), mapRegion);
    DIR *d = opendir(zpath); if (!d) return;
    struct dirent *e; int zMin = INT_MAX, zMax = INT_MIN;
    while ((e = readdir(d))) {
        if (e->d_type == DT_DIR && e->d_name[0] != '.') {
            int z = atoi(e->d_name);
            if (z > 0 && z < 20) { if (z < zMin) zMin = z; if (z > zMax) zMax = z; }
        }
    }
    closedir(d);
    if (zMin <= zMax) { zoomMin = zMin; zoomMax = zMax; zoom = zMax; }
}

static void discoverDefaultPosition() {
    if (!mapRegion[0]) return;
    char zpath[512]; snprintf(zpath, sizeof(zpath), "%s/%s/%d", mapsRoot(), mapRegion, 6);
    DIR *zd = opendir(zpath); if (!zd) return;
    int xMin = INT_MAX, xMax = INT_MIN, yMin = INT_MAX, yMax = INT_MIN;
    struct dirent *xe;
    while ((xe = readdir(zd))) {
        if (xe->d_type != DT_DIR || xe->d_name[0] == '.') continue;
        int tx = atoi(xe->d_name);
        char xpath[600]; snprintf(xpath, sizeof(xpath), "%s/%s", zpath, xe->d_name);
        DIR *xd = opendir(xpath); if (!xd) continue;
        struct dirent *ye;
        while ((ye = readdir(xd))) {
            if (ye->d_type != DT_REG) continue;
            char base[64]; strncpy(base, ye->d_name, sizeof(base)-1); base[sizeof(base)-1]=0;
    ...
    if (xMin <= xMax && yMin <= yMax)
        MapMath::tileToLatLon((xMin+xMax)/2, (yMin+yMax)/2, 6, (float*)&centerLat, (float*)&centerLon);
}

// ---- Tuiles ----
static uint32_t notFoundCache[128]; static int notFoundIdx = 0;
static bool tileExists(int tx, int ty, int z) {
    uint32_t key = ((uint32_t)z<<24)|((uint32_t)(tx&0xFFF)<<12)|(uint32_t)(ty&0xFFF);
    for (int i = 0; i < 128; i++) if (notFoundCache[i] == key) return false;
    char p[512]; snprintf(p, sizeof(p), "%s/%s/%d/%d/%d.jpg", mapsRoot(), mapRegion, z, tx, ty);
    struct stat st;
...
static bool latLonToContPos(float lat, float lon, int *cx, int *cy) {
    int px, py;
    MapMath::latLonToPixel(lat, lon, centerLat, centerLon, zoom, true, centerTX, centerTY, &px, &py);
    *cx = spriteToContX(px);
    *cy = spriteToContY(py);
...
static void closeStationPopup() {
    if (stationPopup && lv_obj_is_valid(stationPopup)) lv_msgbox_close(stationPopup);
...
static void show_station_popup(int stationIdx) {
    ...
    if (stationIdx < 0) {
        // Propre station
        const char *cs = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].callsign.c_str() : "NOCALL";
        const char *ov = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].overlay.c_str() : "/";
        const char *sy = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].symbol.c_str() : "&";
        ...
        char elapsed_str[32];
        if (hours > 0) snprintf(elapsed_str, sizeof(elapsed_str), "%dh%02d", hours, mins % 60);
        else if (mins > 0) snprintf(elapsed_str, sizeof(elapsed_str), "%d min", mins);
        else snprintf(elapsed_str, sizeof(elapsed_str), "%d s", secs);

        // Distance et cap si on a une position GPS
        char dist_str[64] = "";
        if ((gpsLat != 0.0 || gpsLon != 0.0) && (st->latitude != 0.0f || st->longitude != 0.0f)) {
            float dist_m  = calcDist((float)gpsLat, (float)gpsLon, st->latitude, st->longitude);
            float bearing  = calcCourse((float)gpsLat, (float)gpsLon, st->latitude, st->longitude);
            if (dist_m >= 1000.0f)
                snprintf(dist_str, sizeof(dist_str), "\nDist : %.1f km  Cap : %.0f°", dist_m / 1000.0f, bearing);
            else
                snprintf(dist_str, sizeof(dist_str), "\nDist : %.0f m  Cap : %.0f°", dist_m, bearing);
        }
        ...
        snprintf(body, sizeof(body),
                 ...
                 "RSSI : %d dBm\n"
                 "Entendu : %s%s",
                 st->overlay.c_str(), st->symbol.c_str(),
    ...
    lv_obj_set_style_text_color(stationPopup, lv_color_hex(0xffffff), LV_PART_MAIN);
    lv_obj_set_width(stationPopup, 380);
    lv_obj_center(stationPopup);
    lv_obj_add_event_cb(stationPopup, station_popup_deleted_cb, LV_EVENT_DELETE, NULL);
    lv_obj_t *closeBtn = lv_msgbox_add_footer_button(stationPopup, "Close");
    lv_obj_add_event_cb(closeBtn, station_popup_close_btn_cb, LV_EVENT_CLICKED, NULL);
}
...
static void station_longpress_cb(lv_event_t *e) {
    int idx = (int)(intptr_t)lv_event_get_user_data(e);
    if (idx < 0) return;  // own station : pas de message à soi-même
...
// Remplit `path` (taille pathsz) avec le chemin LVGL "A:/..." vers le PNG du symbole
// table = '/' (primary) ou '\\' (alternate), symbol = char APRS (ex: '-', '>', '[')
...
// Crée un conteneur marqueur centré sur (cx, cy) avec l'icône APRS et le callsign
// table/sym : table APRS ('/' ou '\\') et symbole (char) — fallback disque coloré si PNG absent
// overlayChar : caractère alphanumérique à dessiner sur l'icône (alternate table avec overlay),
...
static lv_obj_t *createMarkerObj(lv_obj_t *parent, const char *callsign,
    ...
    if (sym != '\0' && getSymbolPath(table, sym, iconPath, sizeof(iconPath))) {
        ...
        // Overlay character (digi 'D', iGate 'I', etc.) directement sur l'icône, sans fond
    ...
    lv_obj_add_event_cb(m, station_click_cb, LV_EVENT_CLICKED, (void*)(intptr_t)stationIdx);
    lv_obj_add_event_cb(m, station_longpress_cb, LV_EVENT_LONG_PRESSED, (void*)(intptr_t)stationIdx);
    return m;
}

static void createMarkers() {
    if (!mapCont || !mapActive) return;
    deleteMarkers();
    ...
    if (gpsLat != 0.0 || gpsLon != 0.0) {
        ...
        if (latLonToContPos((float)gpsLat, (float)gpsLon, &cx, &cy)) {
            const char *cs  = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].callsign.c_str() : "HOME";
            const char *ov  = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].overlay.c_str() : "/";
            const char *sy  = (!Config.beacons.empty()) ? Config.beacons[myBeaconsIndex].symbol.c_str() : "&";
            char table  = (ov[0] == '/') ? '/' : '\\';
            char sym    = sy[0];
    ...
    for (int i = 0; i < MAP_STATIONS_MAX && markerCount < MAX_MARKERS; i++) {
        ...
        if (st->latitude == 0.0f && st->longitude == 0.0f) continue;

        int cx, cy;
        if (!latLonToContPos(st->latitude, st->longitude, &cx, &cy)) continue;
        ...
        uint32_t elapsed = millis() - st->lastTime;
        lv_color_t fc = (elapsed < 10*60*1000) ? lv_color_hex(0xff6600) : lv_color_hex(0x888888);

        lv_obj_t *m = createMarkerObj(mapCont, st->callsign.c_str(), table, sym, ovChar, fc, cx, cy, i);
        if (m)
...
static void reloadTiles() {
    for (int dy = 0; dy < GRID; dy++) {
        for (int dx = 0; dx < GRID; dx++) {
            ...
            snprintf(p, sizeof(p), "A:%s/%s/%d/%d/%d.jpg", mapsRoot(), mapRegion, zoom, tx, ty);
            if (tileExists(tx, ty, zoom)) lv_image_set_src(tileImg[dy][dx], p);
            else                          lv_image_set_src(tileImg[dy][dx], LV_SYMBOL_IMAGE);
        }
    }
    if (titleLabel) {
    ...
    if (infoLabel) {
        char ib[128];
        snprintf(ib, sizeof(ib), "Lat:%.4f  Lon:%.4f  Stn:%d", centerLat, centerLon, mapStationsCount);
...
void setPosition(double lat, double lon) {
    ...
    for (int i = 0; i < markerCount; i++) {
        if (markers[i].stationIdx == -1 && markers[i].obj && lv_obj_is_valid(markers[i].obj)) {
            int cx, cy;
            latLonToContPos(lat, lon, &cx, &cy);
            lv_obj_set_pos(markers[i].obj, cx - MARKER_W / 2, cy - ICON_SIZE / 2);
            return;
...
void zoomIn() {
    if (zoom < zoomMax) {
        zoom++;
        if (mapActive) { MapMath::latLonToTile(centerLat, centerLon, zoom, &centerTX, &centerTY); reloadTiles(); }
    }
}

void refreshStations() {
    if (mapActive && mapCont) createMarkers();
}
...
static void zoomCb(lv_event_t *e) {
    ...
    MapMath::tileToLatLon(centerTX, centerTY, zoom, (float*)&clat, (float*)&clon);
...
static void mapTouchCB(lv_event_t *e) {
    ...
    if (code == LV_EVENT_PRESSED) {
        ...
        // sinon les tuiles sautent visuellement au premier PRESSING.
        dragLast = p; panActive = true;
        closeStationPopup();
    }
    else if (code == LV_EVENT_PRESSING && panActive) {
        int dx = p.x - dragLast.x, dy = p.y - dragLast.y; dragLast = p;
        dragAccumX += dx; dragAccumY += dy;
        ...
        for (int dy2 = 0; dy2 < GRID; dy2++)
            for (int dx2 = 0; dx2 < GRID; dx2++)
                lv_obj_set_pos(tileImg[dy2][dx2],
...
lv_obj_t *create(lv_obj_t *) {
    ...
    // Reset pan state
    dragAccumX = dragAccumY = 0; panActive = false;
    markerCount = 0;
    stationPopup = nullptr;
    ...
    lv_obj_t *zlm = lv_label_create(zm); lv_label_set_text(zlm, "-"); lv_obj_center(zlm);

    // ---- Conteneur map ----
    mapCont = lv_obj_create(scr);
    lv_obj_set_size(mapCont, CONT_W, MAP_H); lv_obj_set_pos(mapCont, 0, 50);
    ...
    // ── TEST vector : canvas 256×256 en haut-gauche, tile hardcodée z14 Toulouse
    // (le zoom du screen map est reset à zMax raster à chaque entrée, pas
    //  exploitable pour test vector → on hardcode pour voir un rendu détaillé)
    {
        static uint8_t vbuf[LV_CANVAS_BUF_SIZE(256, 256, 32, LV_DRAW_BUF_STRIDE_ALIGN)];
        lv_obj_t *vc = lv_canvas_create(mapCont);
        lv_canvas_set_buffer(vc, vbuf, 256, 256, LV_COLOR_FORMAT_ARGB8888);
        lv_obj_set_pos(vc, 4, 4);
        lv_obj_set_style_border_width(vc, 2, 0);
        lv_obj_set_style_border_color(vc, lv_color_hex(0xff0000), 0);
        const int Z = 14, X = 8265, Y = 6024;  // Foix-ville
        bool ok = MapVector::renderTile(vc, Z, X, Y, 256);
        fprintf(stderr, "MapVector test: tile z=%d x=%d y=%d → %s\n",
                Z, X, Y, ok ? "OK" : "vide");
    }
