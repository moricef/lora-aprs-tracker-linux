...
static lv_timer_t *nav_to_setup_timer = nullptr;
...
static void btn_back_clicked(lv_event_t *e) {
    ...
    UIPopups::closeAll();
    if (nav_to_setup_timer) { lv_timer_del(nav_to_setup_timer); nav_to_setup_timer = nullptr; }
    if (wifi_update_timer)      { lv_timer_del(wifi_update_timer);      wifi_update_timer      = nullptr; }
    if (bluetooth_update_timer) { lv_timer_del(bluetooth_update_timer); bluetooth_update_timer = nullptr; }
    ...
    screen_setup = nullptr;
    UI_SCR_LOAD_ANIM(UIScreens::getMainScreen(), LV_SCR_LOAD_ANIM_MOVE_RIGHT, 100, 0, true);
}
...
void UISettings::createDisplayScreen() {
    ...
    // Disable timeout slider if ECO mode is off (keep visible but grayed out)
    if (!displayEcoMode) {
        lv_obj_add_state(timeout_slider, LV_STATE_DISABLED);
        lv_obj_set_style_text_color(timeout_title, lv_color_hex(0x666666), 0);
        lv_obj_set_style_text_color(timeout_label, lv_color_hex(0x666666), 0);
...
static void ble_setup_timer_cb(lv_timer_t *timer) {
    ESP_LOGI(TAG, "BLE setup: Free heap: %u bytes", 1048576);
...
static void ble_stop_timer_cb(lv_timer_t *timer) {
    ESP_LOGI(TAG, "BLE stop timer: Free heap before stop: %u bytes", 1048576);
    if (Config.bluetooth.useBLE) {
        BLE_Utils::stop();
        ESP_LOGI(TAG, "BLE stop done (deferred). Free heap: %u bytes", 1048576);
    }
...
static pid_t webconf_browser_pid = -1;

static void webconf_close_cb(lv_event_t *e) {
    if (webconf_browser_pid > 0) {
        ::kill(webconf_browser_pid, SIGTERM);
        webconf_browser_pid = -1;
    }
...
void UISettings::openWebConf() {
    ...
    lv_obj_add_event_cb(btn_close, webconf_close_cb, LV_EVENT_CLICKED, NULL);
    lv_obj_t *lbl_btn = lv_label_create(btn_close);
    lv_label_set_text(lbl_btn, "Fermer");
